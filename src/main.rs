...
fn create_docset(indir: &str, name: &str) -> Result {
    ...
    try!(write_file(Path::new(format!("{}.docset/Contents/Info.plist", name).as_str()),
                    format!(include_str!("Info.plist.tmpl"),
                            name = name,
                            identifier = name.replace(" ", "_").replace("-", "_").to_lowercase())
                        .as_ref()));
    try!(write_file(Path::new(format!("{}.docset/icon.png", name).as_str()),
                    include_bytes!("icon.png")));
    try!(write_file(Path::new(format!("{}.docset/icon@2x.png", name).as_str()),
                    include_bytes!("icon@2x.png")));
    ...
    try!(docset_from_rs_doc_tree(Path::new(indir),
                                 format!("{}.docset/Contents/Resources/Documents/", name)
                                     .as_str(),
                                 Path::new(db_path)));
...
fn add_file_to_docset(in_file: &DirEntry, output_prefix: &str, db_path: &Path) -> Result {
    ...
    if in_file.path().extension() == Some(OsStr::new("html")) {
        let mut dom = parse_document(RcDom::default(), ParseOpts::default()).from_utf8()
